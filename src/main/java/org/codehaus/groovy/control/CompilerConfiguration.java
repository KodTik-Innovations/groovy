...
public class CompilerConfiguration {
  ...
  public static final Map<String, Integer> JDK_TO_BYTECODE_VERSION_MAP =
      Maps.of(
          // deenu modify: add java8, java9, java10
          JDK8,
          Opcodes.V1_8,
          JDK9,
          Opcodes.V9,
  ...
  /**
   * A convenience for getting a default configuration. Do not modify it! See {@link
   * #CompilerConfiguration(Properties)} for an example on how to make a suitable copy to modify.
   * But if you're really starting from a default context, then you probably just want <code>
   * new CompilerConfiguration()</code>.
   */
  public static final CompilerConfiguration DEFAULT =
      new CompilerConfiguration() {
        ...
        public Set<String> getDisabledGlobalASTTransformations() {
          return Optional.ofNullable(super.getDisabledGlobalASTTransformations())
        ...
        public Map<String, Object> getJointCompilationOptions() {
          return Optional.ofNullable(super.getJointCompilationOptions())
        ...
        @Override
        public CompilerConfiguration addCompilationCustomizers(
            final CompilationCustomizer... customizers) {
          throw new UnsupportedOperationException();
        }
        ...
        @Override
        public void setDisabledGlobalASTTransformations(
            final Set<String> disabledGlobalASTTransformations) {
          throw new UnsupportedOperationException();
        }
   ...
   * <tt>META-INF/services/org.codehaus.groovy.transform.ASTTransformation</tt> files. By default,
   * none are disabled.
   */
   ...
   *
   * <p>The following system properties are referenced when setting the configuration:
   *
   * <blockquote>
   *
  ...
  public CompilerConfiguration() {
    ...
    setTargetBytecodeIfValid(
        getSystemPropertySafe("groovy.target.bytecode", DEFAULT_TARGET_BYTECODE));
    ...
    handleOptimizationOption(
        RUNTIME_GROOVYDOC, getSystemPropertySafe("groovy.attach.runtime.groovydoc"));
    handleOptimizationOption(
        PARALLEL_PARSE, getSystemPropertySafe("groovy.parallel.parse", "true"));
   ...
   * configuration API.
   *
   * <p>An example of this copy constructor at work:
   *
   * <blockquote>
   *
   * <pre>
  ...
  public CompilerConfiguration(final CompilerConfiguration configuration) {
    ...
    setJointCompilationOptions(
        null != jointCompilationOptions
            ? new HashMap<>(jointCompilationOptions)
  ...
  /**
   * Sets the configuration flags/settings according to values from the supplied {@code Properties}
   * instance or if not found, supplying a default value.
   *
   * <p>Note that unlike {@link #CompilerConfiguration()}, the "defaults" here do <em>not</em> in
   * general include checking the settings in {@link System#getProperties()}. If you want to set a
   * few flags but keep Groovy's default configuration behavior then be sure to make your settings
   * in a {@code Properties} object that is backed by <code>System.getProperties()</code> (which is
   * done using this constructor). That might be done like this:
   *
   * <blockquote>
   *
   * <pre>
   ...
   * </blockquote>
   *
   * And you also have to contend with a possible {@code SecurityException} when getting the system
   * properties (See {@link System#getProperties()}). A safer approach would be to copy a default
  ...
  /**
   * Checks if the specified bytecode version string represents a JDK 1.5+ compatible bytecode
   * version.
   *
  ...
  /**
   * Checks if the specified bytecode version string represents a JDK 1.7+ compatible bytecode
   * version.
   *
   * @param bytecodeVersion The parameter can take one of the values in {@link #ALLOWED_JDKS}.
  ...
  /**
   * Checks if the specified bytecode version string represents a JDK 1.8+ compatible bytecode
   * version.
   *
   * @param bytecodeVersion The parameter can take one of the values in {@link #ALLOWED_JDKS}.
  ...
  /**
   * Checks if the specified bytecode version string represents a JDK 9+ compatible bytecode
   * version.
   *
   * @param bytecodeVersion The parameter can take one of the values in {@link #ALLOWED_JDKS}.
  ...
  /**
   * Checks if the specified bytecode version string represents a JDK 10+ compatible bytecode
   * version.
   *
   * @param bytecodeVersion The parameter can take one of the values in {@link #ALLOWED_JDKS}.
  ...
  /**
   * Checks if the specified bytecode version string represents a JDK 11+ compatible bytecode
   * version.
   *
   * @param bytecodeVersion The parameter can take one of the values in {@link #ALLOWED_JDKS}.
  ...
  /**
   * Checks if the specified bytecode version string represents a JDK 12+ compatible bytecode
   * version.
   *
   * @param bytecodeVersion The parameter can take one of the values in {@link #ALLOWED_JDKS}.
  ...
  /**
   * Checks if the specified bytecode version string represents a JDK 13+ compatible bytecode
   * version.
   *
   * @param bytecodeVersion The parameter can take one of the values in {@link #ALLOWED_JDKS}.
  ...
  /**
   * Checks if the specified bytecode version string represents a JDK 14+ compatible bytecode
   * version.
   *
   * @param bytecodeVersion The parameter can take one of the values in {@link #ALLOWED_JDKS}.
  ...
  /**
   * Checks if the specified bytecode version string represents a JDK 15+ compatible bytecode
   * version.
   *
   * @param bytecodeVersion The parameter can take one of the values in {@link #ALLOWED_JDKS}.
  ...
  /**
   * Checks if the specified bytecode version string represents a JDK 16+ compatible bytecode
   * version.
   *
   * @param bytecodeVersion The parameter can take one of the values in {@link #ALLOWED_JDKS}.
  ...
  /**
   * Checks if the specified bytecode version string represents a JDK 17+ compatible bytecode
   * version.
   *
   * @param bytecodeVersion The parameter can take one of the values in {@link #ALLOWED_JDKS}.
  ...
  /**
   * Checks if the specified bytecode version string represents a JDK 18+ compatible bytecode
   * version.
   *
   * @param bytecodeVersion The parameter can take one of the values in {@link #ALLOWED_JDKS}.
  ...
  /**
   * Method to configure a CompilerConfiguration by using Properties. For a list of available
   * properties look at {@link #CompilerConfiguration(Properties)}.
   *
   * @param configuration The properties to get flag values from.
   */
  ...
  /** Gets the currently configured warning level. See {@link WarningMessage} for level details. */
  ...
  /**
   * Sets the error tolerance, which is the number of non-fatal errors (per unit) that should be
   * tolerated before compilation is aborted.
   */
  public void setTolerance(final int tolerance) {
  ...
  /**
   * Sets the bytecode compatibility level. The parameter can take one of the values in {@link
   * #ALLOWED_JDKS}.
   *
  ...
  private void setTargetBytecodeIfValid(final String version) {
    int index =
        Arrays.binarySearch(
            ALLOWED_JDKS,
            !version.startsWith("1") && !version.startsWith("2") ? "1." + version : version);
    if (index >= 0) {
  ...
  /**
   * Retrieves the compiler bytecode compatibility level. Defaults to the minimum officially
   * supported bytecode version for any particular Groovy version.
   *
  ...
  public final int getBytecodeVersion() {
    ...
    if (bytecodeVersion == null) {
      throw new GroovyBugError(
          "Bytecode version '" + getTargetBytecode() + "' is not supported by the compiler");
    }
  ...
  /**
   * Sets the joint compilation options for this configuration. Using null will disable joint
   * compilation.
   *
  ...
  /**
   * Sets the optimization options for this configuration. No entry or a true for that entry means
   * to enable that optimization, a false means the optimization is disabled. Valid keys are "all"
   * and "int".
   *
   * @param options the options.
  ...
  /**
   * Adds compilation customizers to the compilation process. A compilation customizer is a class
   * node operation which performs various operations going from adding imports to access control.
   *
  ...
  /**
   * Disables the specified global AST transformations. In order to avoid class loading side
   * effects, it is not recommended to use MyASTTransformation.class.getName() but instead directly
   * use the class name as a string. Disabled AST transformations only apply to automatically loaded
   * global AST transformations, that is to say transformations defined in a
   * META-INF/services/org.codehaus.groovy.transform.ASTTransformation file. If you explicitly add a
   * global AST transformation in your compilation process, for example using the {@link
   * org.codehaus.groovy.control.customizers.ASTTransformationCustomizer} or using a {@link
   * org.codehaus.groovy.control.CompilationUnit.IPrimaryClassNodeOperation}, then nothing will
   * prevent the transformation from being loaded.
   *
   * @param disabledGlobalASTTransformations a set of fully qualified class names of global AST
   *     transformations which should not be loaded.
   */
  public void setDisabledGlobalASTTransformations(
      final Set<String> disabledGlobalASTTransformations) {
