...
public abstract class ClassValue<T> {
   ...
   * #get get} method.
   *
   * <p>Normally, this method is invoked at most once per class, but it may be invoked again if
   * there has been a call to {@link #remove remove}.
   *
  ...
  /**
   * Returns the value for the given class. If no value has yet been computed, it is obtained by an
   * invocation of the {@link #computeValue computeValue} method.
   *
   * <p>The actual installation of the value on the class is performed atomically. At that point, if
   * several racing threads have computed values, one is chosen, and returned to all the racing
   * threads.
   *
   * <p>The {@code type} parameter is typically a class, but it may be any type, such as an
   * interface, a primitive type (like {@code int.class}), or {@code void.class}.
   *
   * <p>In the absence of {@code remove} calls, a class value has a simple state diagram:
  ...
  /**
   * Slow tail of ClassValue.get to retry at nearby locations in the cache, or take a slow lock and
   * check the hash table. Called only if the first probe was empty or a collision. This is a
   * separate method, so compilers can process it independently.
   */
  ...
  /**
   * This ClassValue's identity, expressed as an opaque object. The main object {@code
   * ClassValue.this} is incorrect since subclasses may override {@code ClassValue.equals}, which
  ...
  private static ClassValueMap initializeMap(Class<?> type) {
    ...
    synchronized (CRITICAL_SECTION) { // private object to avoid deadlocks
      ...
      if ((map = MAP_CACHE.get(type)) == null) map = new ClassValueMap(type);
  ...
  /**
   * Current version for retrieving this class value from the cache. Any number of computeValue
   * calls can be cached in association with one version. But the version changes when a remove (on
   * any type) is executed. A version change invalidates all cache entries for the affected
   * ClassValue, by marking them as stale. Stale cache entries do not force another call to
   * computeValue, but they do require a synchronized visit to a backing map.
   *
   * <p>All user-visible state changes on the ClassValue take place under a lock inside the
   * synchronized methods of ClassValueMap. Readers (of ClassValue.get) are notified of such state
   * changes when this.version is bumped to a new token. This variable must be volatile so that an
   * unsynchronized reader will receive the notification without delay.
   *
   * <p>If version were not volatile, one thread T1 could persistently hold onto a stale value
   * this.value == V1, while while another thread T2 advances (under a lock) to this.value == V2.
   * This will typically be harmless, but if T1 and T2 interact causally via some other channel,
   * such that T1's further actions are constrained (in the JMM) to happen after the V2 event, then
   * T1's observation of V1 will be an error.
   *
   * <p>The practical effect of making this.version be volatile is that it cannot be hoisted out of
   * a loop (by an optimizing JIT) or otherwise cached. Some machines may also require a barrier
   * instruction to execute before this.version.
   */
   ...
   * </ul>
   *
   * Promises are never put into the cache; they only live in the backing map while a computeValue
   * call is in flight. Once an entry goes stale, it can be reset at any time into the dead state.
   */
  ...
  /**
   * A backing map for all ClassValues, relative a single given type. Gives a fully serialized "true
   * state" for each pair (ClassValue cv, Class type). Also manages an unserialized fast-path cache.
   */
  static class ClassValueMap extends WeakHashMap<ClassValue.Identity, ClassValue.Entry<?>> {
    ...
    /**
     * Number of entries initially allocated to each type when first used with any ClassValue. It
     * would be pointless to make this much smaller than the Class and ClassValueMap objects
     * themselves. Must be a power of 2.
     */
    private static final int INITIAL_ENTRIES = 32;

    /**
     * Build a backing map for ClassValues, relative the given type. Also, create an empty cache
     * array and install it on the class.
     */
    ...
    synchronized <T> ClassValue.Entry<T> finishEntry(
        ClassValue<T> classValue, ClassValue.Entry<T> e) {
      @SuppressWarnings("unchecked") // one map has entries for all value types <T>
    ...
    static <T> ClassValue.Entry<T> probeHomeLocation(
        ClassValue.Entry<?>[] cache, ClassValue<T> classValue) {
    ...
    static <T> ClassValue.Entry<T> probeBackupLocations(
        ClassValue.Entry<?>[] cache, ClassValue<T> classValue) {
      ...
      for (int i = home + 1; i < home + PROBE_LIMIT; i++) {
        ...
        if (classValue.match(e)) {
          ...
          cache[pos2 & mask] =
              ((entryDislocation(cache, pos2, e2) < PROBE_LIMIT)
    ...
    private static int entryDislocation(
        ClassValue.Entry<?>[] cache, int pos, ClassValue.Entry<?> e) {
    ...
    /**
     * Clearing a cache slot risks disconnecting following entries from the head of a non-null run,
     * which would allow them to be found via reprobes. Find an entry after cache[begin] to plug
     * into the hole, or return null if none is needed.
     */
    ...
    /**
     * Store the given entry. Update cacheLoad, and return any live victim. 'Gently' means return
     * self rather than dislocating a live victim.
     */
    private ClassValue.Entry<?> placeInCache(
        ClassValue.Entry<?>[] cache, int pos, ClassValue.Entry<?> e, boolean gently) {
    ...
    /**
     * Note an entry that is about to be overwritten. If it is not live, quietly replace it by null.
     * If it is an actual null, increment cacheLoad, because the caller is going to store something
     * in its place.
     */
